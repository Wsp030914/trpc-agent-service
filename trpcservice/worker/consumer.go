...
func (c *Consumer) executeClaim(ctx context.Context, claim queue.Claim) (bool, error) {
	...
	if runErr != nil || !result.RunnerCompleted {
		if err := c.jobs.Retry(completeCtx, claim, executionFailure(runErr, result)); errors.Is(err, queue.ErrLeaseLost) {
			return true, nil
		} else if err != nil {
...
func executionFailure(err error, result RunResult) error {
	...
	if !result.RunnerCompleted {
		return errors.New("runner completed without completion event")
	}
	return nil
}
