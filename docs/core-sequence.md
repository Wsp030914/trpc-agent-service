...
主链以当前 WeCom/Feishu 长连接为例；HTTP OpenAI 请求在 `Gateway Admission` 处汇合。每条消息都必须携带或生成以下上下文：
...
sequenceDiagram
  ...
  participant S as Redis Session Lease
  R->>W1: pending delivery
  W1->>DB: claim(owner=W1, run_token=t1)
  W1->>S: acquire partition lease
  W1--xW1: process crash / lease stops renewing
  R->>W2: XAUTOCLAIM after min-idle
  W2->>DB: claim only if execution lease expired
  DB-->>W2: new owner/run_token=t2
  W2->>S: acquire same partition after t1 expires
