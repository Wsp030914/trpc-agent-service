...
| R3 mapping/session/reply | normalize、direct/group/topic isolation、identity mapping、async text reply、dedupe/retry | `ChannelInput`、identity/conversation、Reply Outbox；真实文本消息 execution/reply receipt | EXTERNALLY_VERIFIED (real direct messages) | 本次未覆盖群聊/topic、重投、撤回和 provider 限流 |
...
| R5 deployment | Compose、Kustomize、probe、HPA/PDB、single-owner Channel | `compose.yaml`、`deploy/kubernetes`、`scripts/validate-deployment.sh`、deployment Workflow；本机 Compose 实测 | EXTERNALLY_VERIFIED (Compose) | Kubernetes admission、Pods、HPA/PDB 在线行为和 HA 发布未外部验证 |
