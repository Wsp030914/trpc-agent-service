...
状态：`IMPLEMENTED` = 实现存在但证据不完整；`REPO_VERIFIED` = 可由仓库源码、静态检查、单测、集成/E2E 测试或 CI Workflow 核对；`EXTERNALLY_VERIFIED` = 仓库包含可复核的真实外部报告；`EXTERNAL_VERIFICATION_NOT_INCLUDED` = 外部验证不在当前仓库；`NOT_APPLICABLE` = 当前路径不适用。
...
| R1.3 | Gateway/Channel/Worker/Admin/Telemetry 协作 | `cmd/trpc-service/main.go` 的 `gateway`/`channel`/`worker`/`all` roles；Channel role owns adapters, Reply Sender and outbound resolver；Compose/Kustomize | REPO_VERIFIED |
...
| ID | Requirement | Design / implementation / validation | Status |
| --- | --- | --- | --- |
| R3.1 | 至少两类 IM，含微信/企业微信 | WeCom Bot WebSocket + Feishu/Lark WebSocket；adapter tests/deterministic IM workflow | REPO_VERIFIED |
| R3.2 | inbound normalize → ChannelInput/Runner | provider protocol → `ChannelInput` → Gateway Admission → framework Runner | REPO_VERIFIED |
| R3.3 | Agent event → reply | durable event projection；Channel role 的 WeCom/Feishu Reply Sender 消费 Reply Outbox 并发送文本 | REPO_VERIFIED |
| R3.4 | binding、secret、auth、dedupe、identity | binding scope/revision、scoped SecretProvider、官方 WebSocket/SDK protocol auth、HMAC/AEAD、inbox | REPO_VERIFIED |
...
| R3.7 | 真实第三方 IM 账号和网络可用性 | 当前无真实账号、网络、重投或回执报告 | EXTERNAL_VERIFICATION_NOT_INCLUDED |
...
| R4.7 | request/model/tool/IM/Session/Memory metrics | OTel metrics、operations gauges、Grafana dashboard | REPO_VERIFIED |
| R4.8 | callback → Runner → backend → reply trace | W3C traceparent/tracestate 随 Admission、Redis dispatch、Worker、Channel Reply Sender/Reply Outbox 传播 | REPO_VERIFIED |
...
| R5.7 | local/recommended deployment | Compose + Kustomize Gateway/Channel/Worker + probe/HPA/PDB; deployment validation/workflow definitions | IMPLEMENTED |
...
仓库没有外部生产容量、真实 Provider/IM、真实 Kubernetes HA 或真实故障演练证据；这些边界在 `acceptance.md`、`capacity.md`、`deployment.md` 和 `risk-register.md` 中保持一致。
